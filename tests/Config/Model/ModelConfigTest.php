<?php
namespace fetova\Administrator\Tests\Config\Model;
...
class ModelConfigTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp()
	{
		...
		$this->config = m::mock('fetova\Administrator\Config\Model\Config', array($this->validator, array()))->makePartial();
	}
	...
	{
		$stubClass = 'fetova\Administrator\Tests\Config\Model\EloquentStub';
	...
	public function testUpdateModelWithLink()
	{
		...
		$fieldFactory = m::mock('fetova\Administrator\Fields\Factory');
		$fieldFactory->shouldReceive('getEditFieldsArrays')->once();
		$actionFactory = m::mock('fetova\Administrator\Actions\Factory');
	...
	public function testFillModel()
	{
		...
		$field->shouldReceive('getOption')->times(3)->andReturn(false, 'text', false)
				->shouldReceive('fillModel')->once();
		$field_external = m::mock('fetova\Administrator\Fields\Field');
		$field_external->shouldReceive('getOption')->times(3)->andReturn(true, 'belongs_to_many', false);
		$field_setter = m::mock('fetova\Administrator\Fields\Field');
		$field_setter->shouldReceive('getOption')->times(3)->andReturn(false, 'text', true)
					->shouldReceive('fillModel')->once();
		$field_password = m::mock('fetova\Administrator\Fields\Field');
