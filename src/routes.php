...
Route::group(array('prefix' => Config::get('administrator::administrator.uri'), 'before' => 'validate_admin'), function()
{
	...
	Route::get('/', array(
		'as' => 'admin_dashboard',
		'uses' => 'fetova\Administrator\AdminController@dashboard',
	));

	//File Downloads
	Route::get('file_download', array(
		'as' => 'admin_file_download',
		'uses' => 'fetova\Administrator\AdminController@fileDownload'
	));

	//Custom Pages
	Route::get('page/{page}', array(
		'as' => 'admin_page',
		'uses' => 'fetova\Administrator\AdminController@page'
	));
	...
	Route::group(array('before' => 'validate_settings|post_validate'), function()
	{
		...
		Route::group(array('before' => 'csrf'), function()
		{
			...
			Route::post('settings/{settings}/save', array(
				...
				'uses' => 'fetova\Administrator\AdminController@settingsSave'
			));

			//Custom Action
			Route::post('settings/{settings}/custom_action', array(
				'as' => 'admin_settings_custom_action',
				'uses' => 'fetova\Administrator\AdminController@settingsCustomAction'
			));
		});
	});
	...
	Route::get('switch_locale/{locale}', array(
		...
		'uses' => 'fetova\Administrator\AdminController@switchLocale'
	));
	...
	Route::group(array('before' => 'validate_model|post_validate'), function()
	{
		...
		Route::get('{model}', array(
			'as' => 'admin_index',
			'uses' => 'fetova\Administrator\AdminController@index'
		));

		//Get Item
		Route::get('{model}/{id}', array(
			'as' => 'admin_get_item',
			'uses' => 'fetova\Administrator\AdminController@item'
		));
		...
		Route::post('{model}/update_options', array(
			...
			'uses' => 'fetova\Administrator\AdminController@updateOptions'
		));
		...
		Route::post('{model}/rows_per_page', array(
			...
			'uses' => 'fetova\Administrator\AdminController@rowsPerPage'
		));
		...
		Route::group(array('before' => 'csrf'), function()
		{
			...
			Route::post('{model}/{id?}/save', array(
				'as' => 'admin_save_item',
				'uses' => 'fetova\Administrator\AdminController@save'
			));

			//Delete Item
			Route::post('{model}/{id}/delete', array(
				'as' => 'admin_delete_item',
				'uses' => 'fetova\Administrator\AdminController@delete'
			));

			//Get results
			Route::post('{model}/results', array(
				'as' => 'admin_get_results',
				'uses' => 'fetova\Administrator\AdminController@results'
			));

			//Custom Model Action
			Route::post('{model}/custom_action', array(
				'as' => 'admin_custom_model_action',
				'uses' => 'fetova\Administrator\AdminController@customModelAction'
			));

			//Custom Item Action
			Route::post('{model}/{id}/custom_action', array(
				'as' => 'admin_custom_model_item_action',
				'uses' => 'fetova\Administrator\AdminController@customModelItemAction'
			));
		});
	});
});
